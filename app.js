...
const countries = [
  ...
  'Greece','Hungary','Iraq','Italy','Jordan','Lebanon','Libya','Morocco','Netherlands','Pakistan',
  'Poland','Romania','Syria','Turkey','Ukraine','Malta','Spain'
];
...
function detectEntities(text, level) {
  const patterns = [
    ['EMAIL', /\b[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.[A-Za-z]{2,}\b/g],
    ['PHONE', /\+?\d[\d\s().-]{7,}\d/g],
    ['CASE_ID', /\b(?:Case\s*(?:No\.?|Number)?\s*[:#-]?\s*)?[A-Z]{1,4}-?\d{2,4}-\d{2,6}\b/g],
    ['PASSPORT_OR_ID', /\b(?:ID|Passport|Document)\s*(?:No\.?|Number)?\s*[:#-]?\s*[A-Z0-9]{5,20}\b/gi],
    ['ADDRESS', /\b\d{1,4}\s+[A-Z][A-Za-z0-9.'-]+(?:\s+[A-Z][A-Za-z0-9.'-]+){0,4}\s+(?:Street|St|Road|Rd|Avenue|Ave|Boulevard|Blvd|Lane|Ln|Way)\b/g],
    ['DATE_EXACT', new RegExp(`\\b\\d{1,2}\\s+(?:${monthPattern})\\s+\\d{4}\\b`, 'g')],
    ['DATE_EXACT', /\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b/g],
    ['FACILITY', /\b(?:Reception Centre|Closed Controlled Access Centre|CCAC|Camp|Facility)\s+[A-Z][\w-]+(?:\s+[A-Z][\w-]+)*\b/g],
    ['ROUTE', /\b(?:route\s+via\s+[A-Z][a-zA-Z-]+(?:\s*[–-]\s*[A-Z][a-zA-Z-]+)*)\b/gi],
...
function anonymizeText(text, level) {
  const entities = detectEntities(text, level);
  let out = text;
  const replacements = [];
...
async function processTxtFile(file, relativePath, level) {
  const raw = await file.text();
  const anonymized = anonymizeText(raw, level);
  return buildTextOutputs(relativePath, anonymized.text, anonymized.replacements, ['txt', 'docx', 'pdf']);
}

async function processDocxFile(file, relativePath, level) {
  const raw = await extractDocxText(file);
  const anonymized = anonymizeText(raw, level);
  return buildTextOutputs(relativePath, anonymized.text, anonymized.replacements, ['txt', 'docx', 'pdf']);
}

async function processPdfFile(file, relativePath, level) {
  const raw = await extractPdfText(file);
  const anonymized = anonymizeText(raw, level);
  return buildTextOutputs(relativePath, anonymized.text, anonymized.replacements, ['txt', 'docx', 'pdf']);
}

async function processXlsxFile(file, relativePath, level) {
  const buffer = await file.arrayBuffer();
    ...
    for (let row = range.s.r; row <= range.e.r; row += 1) {
      ...
      for (let col = range.s.c; col <= range.e.c; col += 1) {
        ...
        if (typeof cell.v === 'string' && !cell.v.startsWith('=')) {
          const result = anonymizeText(cell.v, level);
          cell.v = result.text;
          cell.w = result.text;
...
async function buildTextOutputs(relativePath, text, replacements, formats) {
  ...
  return { sourceName: relativePath, previewText: text, replacements, downloads };
}
    ...
    card.innerHTML = `
      <div class="result-head">
        <div>
          ...
          <p class="muted">${result.replacements.length} replacements in this file</p>
        </div>
...
async function processOne(entry, level) {
  switch (entry.extension) {
    case 'txt':
      return processTxtFile(entry.file, entry.relativePath, level);
    case 'docx':
      return processDocxFile(entry.file, entry.relativePath, level);
    case 'pdf':
      return processPdfFile(entry.file, entry.relativePath, level);
    case 'xlsx':
      return processXlsxFile(entry.file, entry.relativePath, level);
    default:
      return null;
  }
}
...
async function processAll() {
  ...
  for (let i = 0; i < state.files.length; i += 1) {
    ...
    try {
      const result = await processOne(entry, level);

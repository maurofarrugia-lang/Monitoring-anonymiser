...
const manualTermInput = document.getElementById('manualTermInput');
const addManualTermBtn = document.getElementById('addManualTermBtn');
const addSelectedTextBtn = document.getElementById('addSelectedTextBtn');
const clearManualTermsBtn = document.getElementById('clearManualTermsBtn');
const manualTermsList = document.getElementById('manualTermsList');
...
function resetState() {
  ...
  state.numberApprovalCache = new Map();
  state.manualTerms = [];
...
function placeholderFor(original, category) {
  ...
  switch (category) {
    ...
    case 'MANUAL_TERM':
    case 'NUMBER_SERIES':
      replacement = blankLineFor(original);
      break;
    default:
...
function buildPatterns(active) {
  ...
  if (active.has('CASE_ID')) {
    patterns.push(['CASE_ID', /\b(?:Case|File|Refcom|Reference|Appeal|IPAT\s*reference|IPAT|IPA|Police)\s*(?:No\.?|Number|reference)?\s*[:#-]?\s*[A-Z0-9./-]{2,30}\b/gi]);
    patterns.push(['CASE_ID', /\b(?:IPAT|IPA|Police)\s*(?:No\.?|Number)?\s*[:#-]?\s*\d{2,30}\b/gi]);
  ...
  patterns.push(['NUMBER_SERIES', /\b\d{3,}\b/g]);
  for (const term of state.manualTerms) {
    patterns.push(['MANUAL_TERM', new RegExp(escapeRegExp(term), 'gi')]);
  }
...
function rememberEntityMapping(category, original, replacement) {
  const key = `${category}::${String(original).trim()}`.toLowerCase();
  if (!state.entityMap.has(key)) {
    state.entityMap.set(key, { category, original: String(original).trim(), replacement });
    updateCategoryCount(category);
  }
}

function blankLineFor(value) {
  const trimmed = String(value);
  return trimmed.replace(/[^\s]/g, '_');
}

function addManualTerm(term) {
  const normalized = String(term || '').trim();
  if (!normalized) return false;
  if (state.manualTerms.some(existing => existing.toLowerCase() === normalized.toLowerCase())) return false;
  state.manualTerms.push(normalized);
  state.manualTerms.sort((a, b) => a.localeCompare(b));
  renderManualTerms();
  return true;
}

function removeManualTerm(term) {
  state.manualTerms = state.manualTerms.filter(existing => existing.toLowerCase() !== String(term).trim().toLowerCase());
  renderManualTerms();
}
...
function renderManualTerms() {
  if (!manualTermsList) return;
  manualTermsList.innerHTML = '';
  if (!state.manualTerms.length) {
    manualTermsList.innerHTML = '<span class="muted">No manual redaction terms added yet.</span>';
    return;
  }
  for (const term of state.manualTerms) {
    ...
    item.innerHTML = `${escapeHtml(term)} <span aria-hidden="true">×</span>`;
    item.addEventListener('click', () => removeManualTerm(term));
    manualTermsList.appendChild(item);
  }
}
...
function buildContextSnippet(text, start, end) {
  ...
  const snippet = `${left > 0 ? '…' : ''}${text.slice(left, start)}[${text.slice(start, end)}]${text.slice(end, right)}${right < text.length ? '…' : ''}`;
  return snippet.replace(/\s+/g, ' ').trim();
}

async function approveNumberSeries(entity, fullText, fileLabel = 'document') {
  const key = entity.text.trim();
  if (state.numberApprovalCache.has(key)) return state.numberApprovalCache.get(key);
  const snippet = buildContextSnippet(fullText, entity.start, entity.end);
  const approved = window.confirm(
    `Redact this number series?\n\nFile: ${fileLabel}\nNumber: ${entity.text}\nContext: ${snippet}\n\nPress OK to redact with a blank line. Press Cancel to keep it.`
  );
  const decision = { approved, replacement: approved ? blankLineFor(entity.text) : entity.text };
  state.numberApprovalCache.set(key, decision);
  if (approved) rememberEntityMapping('NUMBER_SERIES', entity.text, decision.replacement);
  return decision;
}

async function anonymizeText(text, level, activeCategories = selectedCategories(), fileLabel = 'document') {
  const entities = detectEntities(text, level, activeCategories);
  let out = text;
  const replacements = [];
  for (const entity of [...entities].sort((a, b) => b.start - a.start)) {
    let replacement;
    if (entity.category === 'NUMBER_SERIES') {
      const decision = await approveNumberSeries(entity, text, fileLabel);
      if (!decision.approved) continue;
      replacement = decision.replacement;
    } else if (entity.category === 'DATE_EXACT') {
      replacement = generaliseDate(entity.text);
      rememberEntityMapping(entity.category, entity.text, replacement);
...
function looksLikeSignatureCue(text) {
  return /(?:\bsignature\b|\bsigned\s+by\b|\bsignature\s+of\b|\bapplicant\s+signature\b|\bofficer\s+signature\b|\/s\/)/i.test(text || '');
}

function addSignatureRedaction(pdfLibPage, rect, cueText, pageNumber) {
  const pageWidth = pdfLibPage.getWidth();
  let x = rect.x + rect.width + 8;
  let width = Math.min(Math.max(120, rect.width * 2.4), Math.max(40, pageWidth - x - 16));
  if (width < 60) {
    x = Math.max(16, Math.min(rect.x, pageWidth - 180));
    width = Math.min(170, pageWidth - x - 16);
  }
  const y = rect.y - Math.max(2, rect.height * 0.18);
  const height = Math.max(18, rect.height * 1.7);
  pdfLibPage.drawRectangle({ x, y, width, height, color: rgb(0, 0, 0), borderColor: rgb(0, 0, 0), borderWidth: 0, opacity: 1 });
  rememberEntityMapping('SIGNATURE', `Page ${pageNumber}: ${cueText.trim()}`, 'BLACK BOX REDACTION');
  return { category: 'SIGNATURE', text: cueText.trim(), replacement: 'BLACK BOX REDACTION' };
}

async function filterApprovedEntities(entities, fullText, fileLabel = 'document') {
  const approved = [];
  for (const entity of entities) {
    ...
    const decision = await approveNumberSeries(entity, fullText, fileLabel);
    if (decision.approved) approved.push(entity);
  }
  return approved;
}
...
async function processTxtFile(file, relativePath, level, activeCategories) {
  ...
  const anonymized = await anonymizeText(raw, level, activeCategories, relativePath);
...
async function processPdfPreserveLayout(file, relativePath, level, activeCategories) {
  ...
  for (let pageIndex = 0; pageIndex < pdfjsDoc.numPages; pageIndex += 1) {
    ...
    for (const item of content.items) {
      ...
      if (looksLikeSignatureCue(previewText)) {
        allReplacements.push(addSignatureRedaction(pdfLibPage, rect, previewText, pageNumber));
      }

      const transformed = await anonymizeText(rawText, level, activeCategories, `${relativePath} · page ${pageNumber}`);
...
async function processPdfBlackout(file, relativePath, level, activeCategories) {
  ...
  for (let pageIndex = 0; pageIndex < pdfjsDoc.numPages; pageIndex += 1) {
    ...
    for (const item of content.items) {
      ...
      const entities = await filterApprovedEntities(detectEntities(rawText, level, activeCategories), rawText, `${relativePath} · page ${pageNumber}`);
      ...
      for (const entity of entities) {
        ...
        rememberEntityMapping(entity.category, entity.text, 'BLACK BAR REDACTION');
      }
    }
    ...
    if (!pageHadMatches && allowOcr && (!content.items.length || !hasMeaningfulText(pagePreview.join(' '), file.name))) {
      ...
      for (const line of ocr.lines || []) {
        ...
        if (looksLikeSignatureCue(lineText)) {
          allReplacements.push(addSignatureRedaction(pdfLibPage, { x, y, width, height }, lineText, pageNumber));
          pageHadMatches = true;
        }
        const entities = await filterApprovedEntities(detectEntities(lineText, level, activeCategories), lineText, `${relativePath} · OCR page ${pageNumber}`);
        if (!entities.length) continue;
        pdfLibPage.drawRectangle({ x, y, width, height, color: rgb(0, 0, 0), borderColor: rgb(0, 0, 0), borderWidth: 0, opacity: 1 });
        for (const entity of entities) {
          allReplacements.push({ ...entity, replacement: 'BLACK BAR REDACTION (OCR)' });
          rememberEntityMapping(entity.category, entity.text, 'BLACK BAR REDACTION (OCR)');
        }
      }
      continue;
    }
...
async function processPdfFile(file, relativePath, level, activeCategories) {
  ...
  const anonymized = await anonymizeText(extracted.text, level, activeCategories, relativePath);
    ...
    for (let row = range.s.r; row <= range.e.r; row += 1) {
      ...
      for (let col = range.s.c; col <= range.e.c; col += 1) {
        ...
        if (typeof cell.v === 'string' && !cell.v.startsWith('=')) {
          const result = await anonymizeText(cell.v, level, activeCategories, `${relativePath} · ${sheetName}!${address}`);
          cell.v = result.text;
          cell.w = result.text;
...
addManualTermBtn?.addEventListener('click', () => {
  if (addManualTerm(manualTermInput?.value)) {
    if (manualTermInput) manualTermInput.value = '';
    setStatus('Manual redaction term added. Click Process files to apply it.', 'success');
  } else {
    setStatus('Enter a new manual word or phrase to redact.', 'warn');
  }
});
addSelectedTextBtn?.addEventListener('click', () => {
  const selected = selectedPreviewText();
  if (!selected) {
    setStatus('Select text inside a preview box first, then add it as a manual redaction term.', 'warn');
    return;
  }
  if (addManualTerm(selected)) {
    setStatus('Selected preview text added as a manual redaction term. Click Process files to apply it.', 'success');
  } else {
    setStatus('That selected term is already in the manual redaction list.', 'info');
  }
});
clearManualTermsBtn?.addEventListener('click', () => {
  state.manualTerms = [];
  renderManualTerms();
  setStatus('Manual redaction terms cleared.', 'info');
});
manualTermInput?.addEventListener('keydown', event => {
  if (event.key === 'Enter') {
    event.preventDefault();
    addManualTermBtn?.click();
  }
});

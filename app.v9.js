...
async function processPdfPreserveLayout(file, relativePath, level, activeCategories) {
  ...
  for (let pageIndex = 0; pageIndex < pdfjsDoc.numPages; pageIndex += 1) {
    ...
    for (const item of content.items) {
      ...
      const width = Math.max(item.width || (rawText.length * 5), 8);
      ...
      pdfLibPage.drawText(transformed.text, {
        x: itemX,
        y: y + Math.max(1, height * 0.15),
        size: Math.max(8, height * 0.85),
        font,
        color: rgb(0, 0, 0),
        maxWidth: width,
        lineHeight: Math.max(8, height * 0.9),
      });
...
async function processPdfBlackout(file, relativePath, level, activeCategories) {
  ...
  for (let pageIndex = 0; pageIndex < pdfjsDoc.numPages; pageIndex += 1) {
    ...
    for (const item of content.items) {
      ...
      const yTop = transform[5];
      const height = Math.max(item.height || Math.abs(transform[3]) || 8, 8);
      const totalWidth = Math.max(item.width || (rawText.length * 5), 8);
      const y = viewport.height - yTop - height;
